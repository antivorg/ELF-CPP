...
void elf_32_parser::print_elf_header(void) {
	...
	std::cout << std::left << "Magic Number: " << std::setfill('0') << std::right;
	for (int i=0; i<10; i++) std::cout << std::setw(2) << std::hex << (unsigned int) elfHeader.e_ident[i] << " ";
	std::cout << std::endl << std::left << std::setfill(' ') << std::setw(36) << "Class:" << "ELF32" << std::endl;
	std::cout << std::setw(36) << "Data:";
	elfHeader.e_ident[EI_DATA_offset] - 1 ? std::cout << "Big Endian" : std::cout << "Little Endian";
	std::cout << std::endl << std::setw(36) << "Version:";
	elfHeader.e_ident[EI_VERSION_offset] == 1 ? std::cout << "1 (current)"
							: std::cout << elfHeader.e_ident[EI_VERSION_offset];
	std::cout << std::endl << std::setw(36) << "OS/ABI:" << EI_OSABI[elfHeader.e_ident[EI_OSABI_offset]];
	std::cout << std::endl << std::setw(36) << "ABI Version" << (int) elfHeader.e_ident[EI_ABIVERSION_offset];
	std::cout << std::endl << std::setw(36) << "Type:" << e_type[elfHeader.e_type] << std::endl;
	std::cout << std::setw(36) << "Machine:" << e_machine[elfHeader.e_machine] << std::endl;
	std::cout << std::setw(36) << "Version:" << "0x" << std::hex << (int) elfHeader.e_ident[EI_VERSION_offset];
	std::cout << std::endl << std::setw(36) << "Entry point address:" << "0x" << std::hex << elfHeader.e_entry;
	std::cout << std::endl << std::setw(36) << "Start of program headers:" << std::dec << (int) elfHeader.e_phoff;
	std::cout << std::endl << std::setw(36) << "Start of section headers:" << (int) elfHeader.e_shoff << std::endl;
	std::cout << std::setw(36) << "Flags:" << "0x" << std::hex << (int) elfHeader.e_flags << std::endl;
	std::cout << std::setw(36) << "Size of this header:" << std::dec << (int) elfHeader.e_ehsize << std::endl;
	std::cout << std::setw(36) << "Size of program headers:" << (int) elfHeader.e_phentsize << std::endl;
	std::cout << std::setw(36) << "Number of program headers:" << (int) elfHeader.e_phnum << std::endl;
	std::cout << std::setw(36) << "Size of section headers:" << (int) elfHeader.e_shentsize << std::endl;
	std::cout << std::setw(36) << "Number of section headers:" << (int) elfHeader.e_shnum << std::endl;
	std::cout << std::setw(36) << "Section header string table index:" << (int) elfHeader.e_shstrndx << std::endl;
}
...
void elf_32_parser::print_sections(void) {
	...
	for (section32_t section : sections) {
		...
		std::cout << std::setw(15) << sectionType[section.sh_type];
		std::cout << std::right << std::setfill('0') << std::setw(8) << std::hex << section.sh_addr;
		std::cout << ' ' << std::setw(6) << section.sh_offset;
		std::cout << ' ' << std::setw(6) << section.sh_size;
		std::cout << ' ' << std::setw(2) << std::hex << section.sh_entsize << ' ';
		std::string flags = "";
		for (const auto &pair : sectionFlags) {
			if (section.sh_flags & pair.first != 0) {
				std::cout << section.sh_flags << '\t' << pair.first << std::endl;
				flags += sectionFlags[pair.first][1];
			}
		}
		std::cout << std::setfill(' ') << std::setw(2) << flags;
		std::cout << std::endl;
	}
}
...
void elf_32_parser::print_symbol_table(void) {
	...
	for (int i=1; i<bytes.size()+1; i++) {
		...
		std::cout << std::setw(2) << std::hex << (unsigned int) bytes[i-1];
		if (i % 4 == 0) std::cout << " ";
